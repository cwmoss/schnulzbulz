...
@define('TINY_TEMPLATE_LFT', '{');
@define('TINY_TEMPLATE_RGT', '}');
...
class tiny_template {
    ...
    static function render_string($str, $data = array()) {
        $tpl = self::compile($str);
    ...
    static function compile($tpl, $level = 0) {
        # print "compiling\n";
        # if(!$level) print "compiling\n";
        $lft = @self::$options['lft'] ?: TINY_TEMPLATE_LFT;
        $rgt = @self::$options['rgt'] ?: TINY_TEMPLATE_RGT;
        $lft = preg_quote($lft, '!');
        $rgt = preg_quote($rgt, '!');
        $compiled = array();
        $count = 0;
        ...
        foreach ($compiled as &$c) {
            if (is_array($c)) $c[1] = self::compile($c[1], $level + 1);
        }
        return array($tpl, $compiled);
    }
}
